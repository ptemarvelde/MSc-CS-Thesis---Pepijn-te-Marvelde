...
\begin{figure}
    ...
    \caption[Evaluation of utility on new scenarios]{Evaluation and comparison of cost model's relative utility on new scenarios. The x-axis shows the relative utility, defined as total time saved, with regard to the test set, for each group of scenario types. A value of 100\% means that the cost estimator performs equally well on the test set as on the training set. In short, utility is defined as $U = \frac{\text{estimator time saved}}{\text{true time saved}}$. Relative utility shown here is the utility on the validation set (synthetic data) divided by utility on the test set (real-world data): $\frac{U_{\text{val}}}{U_{\text{test}}} \times 100$}
