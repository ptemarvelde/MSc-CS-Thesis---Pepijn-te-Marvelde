...
In \autoref{sec:5-gpu-performance-analysis} we have shown that the runtime of the evaluated ML scenarios is dominated by memory cost. To create an accurate prediction of the runtime of a scenario we can use the memory cost as a proxy, which is the approach taken by the analytical estimator. The estimator uses the collected profiling metrics to compute the memory cost of the factorized and materialized scenarios. We then choose the scenario with the lowest predicted runtime.

The workflow for computing the factorized and materialized memory cost is shown in \autoref{fig:5-analytical-architecture}. We first collect the operators used for the model under test. We then use a regressor fit to the collected profiling metrics (as shown in \autoref{fig:5-feature-enrichment}) to predict of the factorized and materialized operators. This step is necessary as we do not have profiling metrics for the scenarios we are testing. Last, the costs for each training type are summed, and choose the scenario with the lowest predicted runtime.
...
\subsection{Final Cost Model: Hybrid}
