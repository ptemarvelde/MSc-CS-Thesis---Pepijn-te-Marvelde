...
To answer the question of which characteristics are most influential in the F/M trade-off, we inspect the final hybrid model. For the GPU leg of the model we evaluate the feature gain. For the CPU leg we reason about the feature coefficients in the Linear Regression model. The results are shown in \autoref{fig:5-feature-importance} and \autoref{fig:5-cpu-feature-importance}.
...
% The most important features for the GPU leg are the memory cost, the number of FLOPs executed, and the arithmetic intensity. This is in line with the results from the GPU performance analysis, where we showed that the runtime of the scenarios is dominated by the time it takes to read from and to memory. The most important features for the CPU leg are the memory cost, the number of FLOPs executed, and the number of non-zero items in the dataset. This is in line with the results from the CPU performance analysis, where we showed that the runtime of the scenarios is dominated by the number of FLOPs executed.


\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{chapters/05_cost_estimation/figures/stat5-feat-importance.pdf}
    \caption{Feature importance of the Hybrid model, Statistical (CPU) leg.}
    \label{fig:5-cpu-feature-importance}
\end{figure}
